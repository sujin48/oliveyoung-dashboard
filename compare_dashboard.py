...
## 5. 장점 키워드별 1등 제품
전달받은 [섹션5 — 장점 키워드 Top 5 완성 표] 데이터를 그대로 마크다운 표로 출력.
컬럼: 장점항목 | 1등 제품(언급률%) | 2등 제품(언급률%) | 카테고리 평균(%)
수치·제품명·순서 절대 수정 금지. 받은 데이터 그대로만 표로 포맷할 것.

## 6. 단점 키워드별 최다 언급 제품
전달받은 [섹션6 — 단점 키워드 Top 5 완성 표] 데이터를 그대로 마크다운 표로 출력.
컬럼: 단점항목 | 최다 언급 제품(언급률%) | 최소 언급 제품(언급률%) | 카테고리 평균(%)
수치·제품명·순서 절대 수정 금지. 받은 데이터 그대로만 표로 포맷할 것.
...
with tab_ai:
    ...
    if st.button("🚀 경쟁 제품 비교 전략 분석 시작", use_container_width=True):
        ...
        with st.spinner("AI 분석 중입니다..."):
            ...
            # ── 섹션 5: 장점 키워드 상위 5개 → 1등/2등/카테고리평균 미리 계산 ──
            pros_avg = pros_matrix.mean(axis=0)                      # 키워드별 카테고리 평균
            top5_pros_kw = pros_avg.sort_values(ascending=False).head(5).index.tolist()

            pros_table_rows = []
            for kw in top5_pros_kw:
                col = pros_matrix[kw].sort_values(ascending=False)
                pros_table_rows.append({
                    '장점항목': kw,
                    '1등제품': col.index[0],
                    '1등언급률': round(col.iloc[0], 1),
                    '2등제품': col.index[1] if len(col) > 1 else '-',
                    '2등언급률': round(col.iloc[1], 1) if len(col) > 1 else 0,
                    '카테고리평균': round(pros_avg[kw], 1),
                })

            # ── 섹션 6: 단점 키워드 상위 5개 → 최다/최소/카테고리평균 미리 계산 ──
            cons_avg = cons_matrix.mean(axis=0)
            top5_cons_kw = cons_avg.sort_values(ascending=False).head(5).index.tolist()

            cons_table_rows = []
            for kw in top5_cons_kw:
                col = cons_matrix[kw].sort_values(ascending=False)
                cons_table_rows.append({
                    '단점항목': kw,
                    '최다언급제품': col.index[0],
                    '최다언급률': round(col.iloc[0], 1),
                    '최소언급제품': col.index[-1],
                    '최소언급률': round(col.iloc[-1], 1),
                    '카테고리평균': round(cons_avg[kw], 1),
                })
...
[섹션5 — 장점 키워드 Top 5 완성 표 (이 데이터를 그대로 표로 출력할 것. 절대 수정·재계산 금지)]
{pros_table_rows}

[섹션6 — 단점 키워드 Top 5 완성 표 (이 데이터를 그대로 표로 출력할 것. 절대 수정·재계산 금지)]
{cons_table_rows}
