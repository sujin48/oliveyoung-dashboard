...
## 1. 시장 한눈에 보기
#### 종합 1위 / 2위 / 3위 (각 1개씩, 근거 수치 포함)
#### 카테고리 전체 평균 (평점 / 재구매율 / 자극률 / 가격만족도지수)

## 2. 피부타입별 추천 매트릭스
표 형식으로 출력. 컬럼: 피부타입 | 1순위(근거) | 2순위(근거) | 3순위(근거) | 비추(근거)
건성 / 지성 / 복합성 / 민감성 / 트러블성 모두 작성.
비추 근거는 반드시 해당 피부타입 리뷰어의 1~2점 비율이 가장 높은 제품을 기준으로 한다.
예: 비추 — ○○크림 (민감성 1~2점 비율 8.3%)
...
with tab_ai:
    ...
    if st.button("🚀 경쟁 제품 비교 전략 분석 시작", use_container_width=True):
        ...
        with st.spinner("AI 분석 중입니다..."):
            ...
            # 종합 순위 Top 3 (평균평점 기준)
            top3_products = summary.sort_values('평균평점', ascending=False).head(3)[
                ['product','평균평점','5점비율(%)','재구매율(%)']
            ].to_dict('records')
