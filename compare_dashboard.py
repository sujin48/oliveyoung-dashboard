...
## 1. 시장 한눈에 보기
#### 종합 1위 / 2위 / 3위 (평균평점 60% + 재구매율 40% 복합점수 기준. 전달받은 [종합 순위 Top 3] 데이터를 그대로 사용하고, 각 제품의 평균평점과 재구매율을 함께 표기할 것.)
#### 카테고리 전체 평균 (평점 / 재구매율 / 자극률 / 가격만족도지수)
...
with tab_ai:
    ...
    if st.button("🚀 경쟁 제품 비교 전략 분석 시작", use_container_width=True):
        ...
        with st.spinner("AI 분석 중입니다..."):
            ...
            summary_rank['종합점수']     = (summary_rank['_rating_norm'] * 0.6
                                            + summary_rank['_rep_norm']   * 0.4).round(1)
            top3_products = summary_rank.sort_values('종합점수', ascending=False).head(3)[
                ['product', '평균평점', '재구매율(%)', '종합점수']
            ].to_dict('records')
...
[종합 순위 Top 3 (평균평점 60% + 재구매율 40% 복합점수 기준 / 종합점수는 0~100 정규화값)]
{top3_products}
...
[카테고리 전체 평균]
평점: {cat_avg_rating} / 재구매율: {cat_avg_rep}% / 자극률: {cat_avg_irr}% / 가격만족도: {cat_avg_price}
